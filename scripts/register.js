var _a, _b;
import User from "../data/classes.js";
localStorage.clear();
let userArr = [];
const initData = () => {
    let xhr = new XMLHttpRequest();
    xhr.open("GET", "../scripts/utils/users.json", true);
    xhr.onload = function () {
...
const rgst_handler = () => {
         ...
         || username.value == "" || password.value == "") 
    {
        ...
        let user = new User(nextId, fname.value, lname.value, Number(age.value), username.value, password.value);
        console.log(user);
        console.log(nextId);
        userArr.push(user);
